...
pub struct Game {
    chesses_textures: Vec<Texture>, //棋盘素材
    ...
    computer_turn: bool,
    selected_chess: Option<POS>, //当前用户点击的棋子
    selected_frame: Texture,
    movable_pos: Vec<MOVE>, //当前选中棋子可走的格子
}
...
fn get_chess_texture<T>(chess: ChessId, texture_creator: &TextureCreator<T>) -> Texture {
    ...
    texture_creator
        .load_texture_bytes(match chess {
            ChessId {
                role: RED,
                kind: ELEPHANT,
            } => {
                load_asset_file!("re.png")
            }
            ChessId {
                role: RED,
                kind: LION,
            } => {
                load_asset_file!("rl.png")
            }
            ChessId {
                role: RED,
                kind: TIGER,
            } => {
                load_asset_file!("rt.png")
            }
            ChessId {
                role: RED,
                kind: PANTHER,
            } => {
                load_asset_file!("rp.png")
            }
            ChessId {
                role: RED,
                kind: WOLF,
            } => {
                load_asset_file!("rw.png")
            }
            ChessId {
                role: RED,
                kind: DOG,
            } => {
                load_asset_file!("rd.png")
            }
            ChessId {
                role: RED,
                kind: CAT,
            } => {
                load_asset_file!("rc.png")
            }
            ChessId {
                role: RED,
                kind: RAT,
            } => {
                load_asset_file!("rr.png")
            }
            ChessId {
                role: BLACK,
                kind: ELEPHANT,
            } => {
                load_asset_file!("be.png")
            }
            ChessId {
                role: BLACK,
                kind: LION,
            } => {
                load_asset_file!("bl.png")
            }
            ChessId {
                role: BLACK,
                kind: TIGER,
            } => {
                load_asset_file!("bt.png")
            }
            ChessId {
                role: BLACK,
                kind: PANTHER,
            } => {
                load_asset_file!("bp.png")
            }
            ChessId {
                role: BLACK,
                kind: WOLF,
            } => {
                load_asset_file!("bw.png")
            }
            ChessId {
                role: BLACK,
                kind: DOG,
            } => {
                load_asset_file!("bd.png")
            }
            ChessId {
                role: BLACK,
                kind: CAT,
            } => {
                load_asset_file!("bc.png")
            }
            ChessId {
                role: BLACK,
                kind: RAT,
            } => {
                load_asset_file!("br.png")
            }
            _ => {
                unreachable!("not found chess asset")
            }
        })
...
impl Game {
    ...
    pub fn new(window: Window, event_pump: EventPump) -> Self {
        ...
        for role in RoleType::iter() {
            for kind in ChessKind::iter() {
                game.chesses_textures.push(get_chess_texture(
                    ChessId {
                        role: *role,
                        kind: *kind,
                    },
        ...
        game.canvas
            .set_logical_size(BOARD_WIDTH, BOARD_HEIGHT)
    ...
    fn get_dst_rect(&self, pos: POS) -> Rect {
        ...
        Rect::new(
            col as i32 * CELL_WIDTH as i32
                + (CELL_WIDTH - CHESS_WIDTH) as i32 / 2
                + Self::CHESS_OFFSET.0,
            row as i32 * CELL_HEIGHT as i32
                + (CELL_HEIGHT - CHESS_HEIGHT) as i32 / 2
                + Self::CHESS_OFFSET.1,
            CHESS_WIDTH,
            CHESS_HEIGHT,
        )
    ...
    fn draw_frame(&mut self, tgt_pos: &Vec<POS>) -> Result<(), String> {
        for pos in tgt_pos {
            self.canvas
                .copy(&self.selected_frame, None, self.get_dst_rect(*pos))?;
        }
        Ok(())
    }
        ...
        for i in 0..ROW_NUM {
            for j in 0..COL_NUM {
                ...
                if chess != EMPTY_CHESS {
                    self.canvas.copy(
                        &self.chesses_textures[chess.get_chess_idx()],
                        None,
                        self.get_dst_rect(to_pos(&(i, j))),
                    )?;
                }
            }
        }
