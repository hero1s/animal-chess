...
impl AlphaBeta {
    ...
    fn get_history_score(&mut self, mv: MOVE) -> &mut ScoreType {
        ...
        &mut self.history_table[self.board.borrow().chesses[src.0][src.1].get_chess_idx()][dst.0]
            [dst.1]
    }
    ...
    pub fn search_main(&mut self) -> MOVE {
        ...
        for d in 1..=MAX_DEPTH {
            if now.elapsed().as_micros() as i32 >= timeout {
                break;
            }
